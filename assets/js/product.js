	...
	if(user)
	{
		...
		return firebase.database().ref('/products/' + product).once('value').then(function(snapshot) 
		{
			...
			snapshot.forEach((_child) => 
			{
				...
				//Replace below code
				if(_child.key === "productDescription")
				{
						description = _child.val();
				}
				if(_child.key === "productDirections")
				{
						directions = _child.val();
				}
				if(_child.key === "productIngredients")
				{
		...
		document.getElementById("productDirections").innerText = directions;
