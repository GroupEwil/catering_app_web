...
function Add(id, product, quantity, price, lastID, type)
{
	...
	newRow.id = lastID;
	newRow.setAttribute("style", "color: rgb(255,255,255);");
...
function PlaceOrder()
{
	...
	var dropdownOrderType = document.getElementById('dropdownOrderType');
	var opt = dropdownOrderType.options[dropdownOrderType.selectedIndex];
	var eventType = opt.text;
	...
	newOrderRef.set({
		  ...
		  return firebase.database().ref('/users/' + user.uid).child("cart").once('value').then(function(snapshot) 
		{
			snapshot.forEach((cartItem) =>
				{
					AddItemToOrder(orderID, cartItem.key, cartItem.child("quantity").val());
				});
				var cartRef = firebase.database().ref('users').child(user.uid).child('cart').remove();
...
{
	var obj = firebase.database().ref('/orders').child(orderID).child('cart').child(productID).set({
				quantity: quantity
			}, function(error) 
			{
				if (error) 
